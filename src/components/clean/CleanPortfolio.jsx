          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer whitespace-nowrap shrink-0 ${
                filter === cat.id
                  ? 'bg-gradient-to-r from-cyan-500 to-blue-600 light:from-cyan-600 light:to-blue-700 text-slate-950 light:text-white font-bold shadow-md'
                  : 'bg-slate-900 light:bg-white border border-slate-800 light:border-slate-300 text-slate-400 light:text-slate-700 hover:text-white light:hover:text-slate-900 hover:border-slate-700'
              }`}
            >
              {cat.label}
            </button>
          ...
          return (
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden bg-slate-950 light:bg-slate-100 flex items-center justify-center border-b border-slate-800/60 light:border-slate-200">
                {hasValidImage ? (
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-slate-900 to-slate-950 light:from-slate-100 light:to-slate-200 border border-slate-800 light:border-slate-200 flex flex-col items-center justify-center p-4 relative overflow-hidden group-hover:border-cyan-500/40 transition-colors">
                    ...
                    <div className="p-3.5 rounded-2xl bg-cyan-500/10 light:bg-white border border-cyan-500/30 light:border-slate-300 text-cyan-400 light:text-cyan-700 mb-2 group-hover:scale-110 transition-transform shadow-sm">
                      <IconComp className="w-6 h-6" />
                    </div>
                    
                    <span className="text-[10px] font-mono text-cyan-400 light:text-cyan-700 font-bold uppercase tracking-wider">
                      {project.client}
                    </span>
                  </div>
                )}
                ...
                <div className="absolute top-4 left-4 z-10">
                  <span className="px-3 py-1 rounded-full text-[10px] font-mono font-bold bg-slate-950/90 light:bg-white/95 border border-cyan-500/40 light:border-slate-300 text-cyan-300 light:text-cyan-800 backdrop-blur-md shadow-sm">
              ...
              <div className="p-5 md:p-6 space-y-3">
                <div className="text-xs text-cyan-400 light:text-cyan-700 font-mono font-medium">{project.client} • {project.year}</div>
                ...
                <div className="flex flex-wrap gap-1.5 pt-2">
                  {project.tags.map((tag, idx) => (
                    <span key={idx} className="px-2 py-0.5 rounded text-[10px] font-mono bg-slate-950 light:bg-slate-100 text-slate-400 light:text-slate-700 border border-slate-800 light:border-slate-200">
                      #{tag}
                    </span>
                  ))}
                </div>
              ...
              {/* Action Footer */}
              <div className="px-5 md:px-6 py-3.5 bg-slate-950/80 light:bg-slate-50 border-t border-slate-800 light:border-slate-200 flex items-center justify-between text-xs text-cyan-400 light:text-cyan-700 font-semibold">
