...
export default function BehanceModal({ project, isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/85 backdrop-blur-xl animate-fade-in overflow-y-auto">
      <div className="relative w-full max-w-[92vw] sm:max-w-3xl max-h-[85vh] rounded-2xl sm:rounded-3xl bg-slate-950 border border-purple-500/40 overflow-hidden shadow-2xl flex flex-col justify-between my-auto">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-800/80">
            <div className="flex flex-wrap gap-1.5">
              {project.tags.map((t, idx) => (
                <span key={idx} className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded bg-slate-900 text-[10px] sm:text-xs font-mono text-purple-300 border border-slate-800">
                  #{t}
                </span>
              ))}
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-3.5 py-1.5 sm:px-5 sm:py-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-indigo-600 text-white font-bold text-[10px] sm:text-xs shadow-md hover:scale-105 transition-all flex items-center gap-1.5 cursor-pointer ml-auto"
            >
